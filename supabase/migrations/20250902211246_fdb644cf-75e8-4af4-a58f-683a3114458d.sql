...
AS $$
BEGIN
  -- Log access to sensitive tables for monitoring
...
-- Apply security logging triggers to sensitive tables
...
CREATE TRIGGER secure_messages_log
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.messages
  FOR EACH ROW
  EXECUTE FUNCTION public.secure_data_access_log();