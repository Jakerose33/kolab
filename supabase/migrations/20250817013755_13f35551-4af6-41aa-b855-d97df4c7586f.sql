-- **SECURITY FIX: Event Authorization and Data Protection (Step by Step)**
...
-- 1. First, let's delete orphaned events since we can't assign them to a valid owner
DELETE FROM public.events WHERE organizer_id IS NULL;

-- 2. Now we can safely make organizer_id NOT NULL
