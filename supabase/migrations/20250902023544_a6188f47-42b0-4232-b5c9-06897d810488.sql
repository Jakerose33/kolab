...
USING (true)
WITH CHECK (
  -- This policy applies to SELECT, but we include WITH CHECK for clarity
  -- The actual field filtering is handled by the privacy-aware functions
  true
);

-- However, we need to limit what fields are actually exposed
-- Let's create a view for public profile access that respects privacy
...
-- First, let's update the get_user_privacy_setting function to handle 'show_bio'
...
AS $function$
  SELECT 
    CASE setting_name
      ...
      WHEN 'show_bio' THEN COALESCE(ps.show_interests, false) -- Bio private by default
      ELSE false
    END
...
-- Add bio privacy setting to privacy_settings table
...
-- (Views inherit RLS from underlying tables, but this is for clarity)
