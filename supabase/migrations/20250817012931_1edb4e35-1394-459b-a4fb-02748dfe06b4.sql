-- **SECURITY FIX: Implement Privacy-Aware Profile Access (Corrected)**
...
-- 4. Create new restrictive RLS policy for basic public info only
-- (Note: "Users can view their own complete profile" already exists)
...
-- 7. Update the existing profiles RLS policies to be more restrictive
-- We need to limit what fields can actually be accessed by the "basic public info" policy
-- The current policy allows access to ALL fields, which is the security issue

-- Since PostgreSQL RLS doesn't support column-level restrictions directly,
-- the application layer must use the get_profile_with_privacy() function
-- for proper privacy-aware profile access